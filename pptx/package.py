...
from .parts.media import Media, MediaPart
...
class Package(OpcPackage):
    ...
    def get_or_add_media_part(self, media_file, content_type):
        """Return a |MediaPart| object containing the media in *media_file*.

        If a media part for this bytestream ("file") is already present in
        this package, it is reused, otherwise a new one is created.
        """
        return self._media_parts.get_or_add_media_part(
    ...
    def next_media_partname(self, ext):
        ...
        sequence numbers are reused. *ext* is used as the extention on the
        returned partname.
        """
    ...
    def _media_parts(self):
        """
        |_MediaParts| object providing access to the media parts in this
        package.
        """
...
class _MediaParts(object):
    ...
    def get_or_add_media_part(self, media_file, content_type):
        ...
        *media_file* must be a filesystem path (string). The MIME type of the
        media must be provided as *content_type* as |pp| has no way of
        determining it from the file. If this package already contains
        a media part for the same bytestream, that instance is returned,
        otherwise a new media part is created.
        """
        media = Media.from_file(media_file, content_type)
